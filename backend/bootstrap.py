...
from services import roll_cost_history
...
async def backfill_costing_data() -> None:
    ...
    2) inventory_rolls **yang BELUM PERNAH dinilai** (`base_unit_cost` & `unit_cost`
       sama-sama tidak ada / null) → isi dari products.harga_pokok;
       unit_cost = base_unit_cost + landed_cost_total (default 0).

    INV-GL-DRIFT (akar masalah CV Kanda Suka, ditutup 2026-06)
    ---------------------------------------------------------
    Versi lama menganggap `base_unit_cost == 0` sebagai "belum terisi" dan MENGISINYA
    dari harga pokok produk pada SETIAP kali backend hidup. Nol yang SENGAJA — roll
    retur pelanggan berkondisi `damaged`, roll yang sudah dihapus-buku (`writeoff`),
    atau roll yang dinilai ulang oleh retur antar-PT — karena itu dihidupkan kembali
    menjadi Rp 90.000/unit **tanpa satu pun jurnal 1-1300**. Itulah selisih
    Rp 900.000 di buku Kanda: bukan transaksi yang lupa dijurnal, melainkan migrasi
    startup yang menaikkan nilai persediaan diam-diam. Karena tidak ada galat dan
    tidak ada layar merah, ia bertahan berbulan-bulan dan setiap true-up hanya
    menutupinya sampai backend hidup lagi.

    Sekarang yang dianggap "belum pernah dinilai" hanya field yang MEMANG TIDAK ADA
    (atau null), dan roll yang nilainya keputusan orang (`cost_basis`, `writeoff_*`,
    kondisi `damaged`) tidak pernah disentuh.
    """
    ...
    belum_dinilai = {
        "$and": [
            {"$or": [{"base_unit_cost": None}, {"base_unit_cost": {"$exists": False}}]},
            {"$or": [{"unit_cost": None}, {"unit_cost": {"$exists": False}}]},
            # Nilai yang merupakan KEPUTUSAN (dihapus-buku, retur cacat, revaluasi
            # antar-PT) bukan "data cost yang hilang" — jangan pernah ditimpa.
            {"cost_basis": {"$exists": False}},
            {"writeoff_je_id": {"$exists": False}},
            {"condition": {"$ne": "damaged"}},
        ]
    }
    cursor = db.inventory_rolls.find(
        belum_dinilai, {"_id": 0, "id": 1, "product_id": 1, "landed_cost_total": 1,
                        "roll_no": 1, "unit_cost": 1, "base_unit_cost": 1,
                        "length_remaining": 1, "owner_entity_id": 1})
    async for r in cursor:
        ...
        await roll_cost_history.record(
            r, round(base + landed, 4), "startup_backfill", actor="sistem (migrasi startup)",
            note="roll belum pernah dinilai — HPP diisi dari harga pokok produk")
    ...
    if await db.special_orders.count_documents({"entity_id": entity_id}) == 0:
        ...
        if cust:
            ...
            for s in samples:
                ...
                # B6 DIBAYAR (2026-08-25) — nomor dokumen dibentuk dari TANGGAL
                # DOKUMEN, bukan hari ini. Sebelumnya dokumen "berumur 9 hari"
                # bernomor `SORD-<hari-ini>-0002`: data demo tidak konsisten dengan
                # dirinya sendiri, dan pemeriksa penomoran akan mengira ada bug.
                number = await generate_special_order_number(dibuat)
                doc = {
                ...
                # B1 — dokumen yang MENUNGGU keputusan wajib menyebut kapan ia mulai
                # menunggu (field yang benar-benar dibaca `AGING_META`), bukan
                # membiarkan papan menebak dari `created_at`.
                if s["status"] == "pending_approval":
                    doc["approval_requested_at"] = dibuat
                    doc["required_approval_role"] = "manager"
                await db.special_orders.insert_one(doc)
    # --- Sales Returns ---
...
async def run_bootstrap() -> None:
    ...
    # B1 (2026-08-25) — dokumen PO custom LAMA yang menunggu keputusan diberi
    # `approval_requested_at` dari `status_history` (bukan ditebak dari `created_at`).
    try:
        from services.special_order_service import ensure_approval_requested_at
        await ensure_approval_requested_at()
    except Exception as exc:  # noqa: BLE001 — jangan gagalkan startup
        print(f"[bootstrap] backfill approval_requested_at skip: {exc}")
    # INV-HIST-01 (2026-06) — riwayat status lot warisan (`at`/`actor`) disatukan ke
    # bentuk kanonik (`timestamp`/`user`) supaya pembaca lintas koleksi tidak menebak.
    try:
        import sys as _sys
        from pathlib import Path as _Path
        _sys.path.insert(0, str(_Path(__file__).resolve().parents[1] / "scripts"))
        from migrate_lot_status_history_keys import migrate as _migrate_lot_hist
        await _migrate_lot_hist(db)
    except Exception as exc:  # noqa: BLE001 — jangan gagalkan startup
        print(f"[bootstrap] migrasi bentuk status_history lot skip: {exc}")
