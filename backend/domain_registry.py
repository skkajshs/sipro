...
# ── FASE S — JENIS SAMPLING (master `sample_types`) ──────────────────────────
# KENAPA DAFTAR INI TUMBUH DARI 3 BARIS PENDEK MENJADI BARIS BER-FIELD
# Sebelum FASE S, "jenis sample" hanyalah label. Dua kelakuan penting justru
# tertanam di `if` yang tersebar:
#     if stype == "proofing": wajib kode desain     → services/rnd_sample_service
#     RoundMeasurements = 5 field TETAP di skema     → schemas_rnd
# Akibatnya nyata: menambah jenis **handfeel** (permintaan pemilik) berarti
# menyunting kode di dua tempat, dan hasil ukur yang relevan untuk handfeel
# (`gsm_actual`, `lebar`, `shrinkage_pct`, `handfeel_score`) tidak punya kolom
# sementara `delta_e` yang tak relevan tetap diminta. Jadi dua kelakuan itu
# dipindahkan menjadi DATA di baris master:
#     requires_design      → jenis ini menuntut kode desain (dulu `if == proofing`)
#     measurement_fields   → hasil ukur yang WAJIB diisi saat menutup round
#     applies_to_lines     → [] berarti SEMUA lini (aturan yang sama dengan FASE L)
#
# NILAI DI SINI = BENIH. Nilai HIDUP ada di koleksi `sample_types` (bisa ditambah
# pemilik tanpa programmer) dan dibaca lewat `services/master_registry.py`.
#
# Keputusan pemilik #4 (§12 RENCANA_EKSEKUSI_MD_ERP.md): `bulk_sample`
# **DINONAKTIFKAN** (`active=False`) — TIDAK dihapus dari enum, supaya dokumen lama
# (kalau kelak ada) tetap terbaca dan laporannya tidak kehilangan barisnya.
SAMPLE_TYPES: List[Dict[str, Any]] = [
    {"value": "labdip", "label": "Labdip (kain polos)", "code": "labdip",
     "name": "Labdip (kain polos)", "seq": 10, "active": True,
     "requires_design": False, "applies_to_lines": [],
     "measurement_fields": ["delta_e", "colorfastness_wash", "colorfastness_rub"],
     "notes": "Cocokkan WARNA pada kain polos. Yang diukur: selisih warna ΔE dan "
              "ketahanan warna (cuci & gosok)."},
    {"value": "handfeel", "label": "Handfeel (rasa & konstruksi)", "code": "handfeel",
     "name": "Handfeel (rasa & konstruksi)", "seq": 20, "active": True,
     "requires_design": False, "applies_to_lines": [],
     "measurement_fields": ["gsm_actual", "lebar", "shrinkage_pct", "handfeel_score"],
     "notes": "Menilai RASA kain & konstruksinya — gramasi, lebar, susut, dan skor "
              "handfeel 1–5. Sengaja dipisah dari labdip: skor 90 pada labdip berarti "
              "warnanya tepat, pada handfeel berarti rasanya tepat."},
    ...
    {"value": "bulk_sample", "label": "Bulk sample", "code": "bulk_sample",
     ...
     "requires_design": False, "applies_to_lines": [],
     "measurement_fields": [],
     "notes": "DINONAKTIFKAN (keputusan pemilik #4, 2026-08-19): nol dokumen "
              "memakainya. Tidak dihapus dari enum supaya dokumen lama tetap terbaca."},
]

# ── FASE I — ALASAN KELUHAN RETUR (master `complaint_reasons`) ───────────────
# Pemilik meminta lembar retur menyebut SEBABNYA, bukan hanya "barang kembali".
# Sebabnya dipakai tiga pertanyaan yang selama ini tak terjawab: (a) supplier/lini
# mana yang paling sering bermasalah, (b) keluhan mana yang berujung klaim, dan
# (c) apakah pemeriksaan inspeksi menyetujui keluhan pelanggan.
#
# `needs_inspection=True` = keluhan yang WAJIB dibuktikan lewat inspeksi fisik
# sebelum kredit/ganti diberikan (kelas keluhan yang bisa diperdebatkan). Keluhan
# administratif seperti salah kirim tidak perlu diperiksa poin cacatnya.
# `claimable=True` = wajar diteruskan sebagai klaim ke supplier/makloon.
COMPLAINT_REASONS: List[Dict[str, Any]] = [
    {"value": "warna_beda", "label": "Warna tidak sesuai", "code": "warna_beda",
     "name": "Warna tidak sesuai", "seq": 10, "active": True,
     "needs_inspection": True, "claimable": True, "applies_to_lines": [],
     "notes": "Warna kain berbeda dari sample/labdip yang sudah di-ACC. Diperiksa "
              "dengan membandingkan roll terhadap sample acuan."},
    {"value": "kotor_stain", "label": "Kotor / kena noda", "code": "kotor_stain",
     "name": "Kotor / kena noda", "seq": 20, "active": True,
     "needs_inspection": True, "claimable": True, "applies_to_lines": [],
     "notes": "Noda oli, jamur, atau kotoran proses. Dihitung sebagai cacat 4-point."},
    {"value": "sobek_lubang", "label": "Sobek / berlubang", "code": "sobek_lubang",
     "name": "Sobek / berlubang", "seq": 30, "active": True,
     "needs_inspection": True, "claimable": True, "applies_to_lines": [],
     "notes": "Kerusakan fisik kain. Poin cacat 4-point menentukan turun grade."},
    {"value": "salah_kirim", "label": "Salah kirim artikel/warna", "code": "salah_kirim",
     "name": "Salah kirim artikel/warna", "seq": 40, "active": True,
     "needs_inspection": False, "claimable": False, "applies_to_lines": [],
     "notes": "Kesalahan administrasi/pengambilan barang — bukan mutu kain, jadi "
              "tidak perlu inspeksi poin cacat dan tidak diklaim ke supplier."},
    {"value": "kurang_yard", "label": "Kekurangan meteran", "code": "kurang_yard",
     "name": "Kekurangan meteran", "seq": 50, "active": True,
     "needs_inspection": True, "claimable": True, "applies_to_lines": [],
     "notes": "Panjang aktual kurang dari yang tertulis di surat jalan. Dibuktikan "
              "dengan pengukuran ulang saat inspeksi."},
    {"value": "handfeel", "label": "Rasa kain tidak sesuai", "code": "handfeel",
     "name": "Rasa kain tidak sesuai", "seq": 60, "active": True,
     "needs_inspection": True, "claimable": True, "applies_to_lines": [],
     "notes": "Gramasi, lebar, susut, atau handfeel menyimpang dari sample yang "
              "di-ACC. Bawaan kebijakan: MEMPERINGATKAN (keputusan pemilik #5)."},
    {"value": "lain", "label": "Lain-lain (tulis di catatan)", "code": "lain",
     "name": "Lain-lain (tulis di catatan)", "seq": 90, "active": True,
     "needs_inspection": False, "claimable": False, "applies_to_lines": [],
     "notes": "Wadah untuk keluhan yang belum punya kodenya. Kalau baris ini sering "
              "terpakai, itu isyarat pemilik perlu menambah kode baru — bukan alasan "
              "membiarkan sebabnya tak terbaca."},
]

# ── FASE S — KAMUS HASIL UKUR SAMPLE ────────────────────────────────────────
# `sample_types.measurement_fields` hanya menyebut NAMA field. Bentuknya (label
# manusia, satuan, batas wajar) hidup di sini supaya SATU sumber melayani tiga
# pemakai yang dulu punya salinan sendiri: validator server, form setor hasil, dan
# baris ringkasan di panel sample. Batas `min`/`max` dipakai menolak salah ketik
# yang tampak wajar (ΔE 120, skor handfeel 50) — bukan untuk menilai mutu.
SAMPLE_MEASUREMENTS: List[Dict[str, Any]] = [
    {"value": "delta_e", "label": "ΔE (selisih warna)", "unit": "", "min": 0, "max": 100,
     "hint": "Makin kecil makin dekat ke warna target. Umumnya ACC di bawah 1.5."},
    {"value": "colorfastness_wash", "label": "Tahan cuci", "unit": "skala 1–5",
     "min": 1, "max": 5, "hint": "Grey scale AATCC/ISO — 5 = tidak berubah."},
    {"value": "colorfastness_rub", "label": "Tahan gosok", "unit": "skala 1–5",
     "min": 1, "max": 5, "hint": "Crocking test — 5 = tidak menodai."},
    {"value": "gsm_actual", "label": "Gramasi aktual", "unit": "g/m²", "min": 1,
     "max": 2000, "hint": "Dibandingkan dengan gramasi target di spesifikasi."},
    {"value": "lebar", "label": "Lebar kain", "unit": "cm", "min": 1, "max": 500,
     "hint": "Lebar jadi setelah proses — menentukan kebutuhan meter."},
    {"value": "shrinkage_pct", "label": "Susut", "unit": "%", "min": 0, "max": 100,
     "hint": "Susut setelah cuci. Nilai besar mengubah kebutuhan bahan."},
    {"value": "handfeel_score", "label": "Skor handfeel", "unit": "skala 1–5",
     "min": 1, "max": 5, "hint": "Penilaian rasa kain oleh QC sample."},
    {"value": "repeat_cm", "label": "Panjang repeat", "unit": "cm", "min": 0.1,
     "max": 500, "hint": "Panjang satu ulangan motif — wajib sama dengan desain."},
    {"value": "register_mm", "label": "Ketepatan register", "unit": "mm", "min": 0,
     "max": 100, "hint": "Selisih tumpang-tindih antar-warna cetak."},
]

# ── FASE S — TUJUAN PENGIRIMAN SAMPLE JADI ──────────────────────────────────
# Keputusan pemilik (sesi ini): tujuannya WAJIB dipilih, dan pilihannya empat.
# Alasan `internal` ada: sample yang tidak dikirim ke siapa pun tetap harus punya
# tempat tercatat — kalau tidak, petugas akan memilih "pelanggan" supaya tombolnya
# jalan, dan laporan pengiriman sample berhenti bisa dipercaya.
SAMPLE_DELIVER_TARGETS: List[Dict[str, Any]] = [
    {"value": "customer", "label": "Pelanggan",
     "description": "Dikirim ke pelanggan untuk persetujuan."},
    {"value": "sales", "label": "Sales / Marketing",
     "description": "Dipegang sales sebagai alat jual."},
    {"value": "supplier", "label": "Supplier (kirim balik)",
     "description": "Dikembalikan ke supplier sebagai acuan produksi."},
    {"value": "internal", "label": "Arsip internal / QC",
     "description": "Disimpan sendiri sebagai acuan pembanding saat inspeksi."},
]
...
PRODUCT_LINES: List[Dict[str, Any]] = [
    {"value": "woven", "label": "Woven (Tenun)", "fabric_type_required": "woven",
     ...
     # FASE P (2026-08-21) — kode tahap di sini WAJIB kode **TAHAP PROSES**
     # (`PROCESS_STAGES` di bawah), bukan kode **tahap bahan** (`MATERIAL_STAGES`).
     # Dulu baris ini berbunyi `["yarn", …]`: `yarn` adalah tahap BAHAN (keadaan
     # kain), sementara langkah kerjanya bernama `benang` ("Benang (bahan masuk)").
     # Akibatnya papan PO menampilkan chip "Yarn" hasil menebak dari kodenya
     # (label master tak ditemukan) — satu kata untuk dua arti, kelas cacat yang
     # sama dengan `line_code` vs `fabric_type`. Dijaga `INV-STAGE-01`.
     "stage_sequence": ["benang", "tenun", "celup", "inspect"],
     # FASE S (keputusan pemilik #3, 2026-08-19): bawaan woven/knit = labdip +
     # handfeel. Boleh lebih dari satu, dan pemilik bisa mengubahnya sendiri di
     # master Lini Produk tanpa programmer.
     "sample_types_default": ["labdip", "handfeel"],
    ...
    {"value": "knit", "label": "Knit (Rajut)", "fabric_type_required": "knit",
     ...
     "stage_sequence": ["benang", "rajut", "celup", "inspect"],
     "sample_types_default": ["labdip", "handfeel"],
...
PROCESS_STAGES: List[Dict[str, Any]] = [
    ...
    {"value": "proofing", "label": "Proofing (sample printing)", "code": "proofing",
     ...
     "material_flow_default": "service_only", "active": True,
     "notes": "Uji cetak sebelum produksi (FASE S: `md_samples.sample_types[]` memuat `proofing`)."},
...
ENUMS: Dict[str, Dict[str, Any]] = {
    ...
    # ── FASE S — JENIS SAMPLING. `values` di sini = BENIH; `/api/enums` menimpanya
    # dengan nilai HIDUP dari master `sample_types` (services/master_registry.py),
    # sehingga jenis yang baru ditambah pemilik langsung muncul di form permintaan
    # sample & pemilih jenis tanpa ubah kode.
    "sample_type": {"label": "Jenis Sampling", "ps": "PS-12", "decision": "S-01",
                    "note": "FASE S — dipakai `md_samples.sample_types[]` (BOLEH LEBIH "
                            "DARI SATU) dan `rounds[].type_code`: labdip (warna kain "
                            "polos) · handfeel (rasa & konstruksi) · proofing (printing, "
                            "`requires_design=true`) · bulk_sample (dinonaktifkan). "
                            "`requires_design` & `measurement_fields` adalah DATA di "
                            "baris master — dulu keduanya `if` di dalam kode.",
                    "values": SAMPLE_TYPES, "in_use": True},
    # ── FASE I — ALASAN KELUHAN RETUR. `values` di sini = BENIH; `/api/enums`
    # menimpanya dengan nilai HIDUP dari master `complaint_reasons`
    # (services/master_registry.py), sehingga kode keluhan yang ditambah pemilik
    # langsung muncul di form retur tanpa ubah kode.
    "complaint_reason": {"label": "Alasan Keluhan Retur", "ps": "FASE I",
                         "decision": "I-01",
                         "note": "FASE I — dipakai `sales_returns.complaint_code` "
                                 "(+`complaint_note`). `needs_inspection` menentukan "
                                 "apakah keluhan wajib dibuktikan lewat inspeksi fisik; "
                                 "`claimable` menentukan apakah wajar diteruskan sebagai "
                                 "klaim ke supplier. Keduanya DATA di baris master — "
                                 "bukan `if` di dalam kode.",
                         "values": COMPLAINT_REASONS, "in_use": True},
    "sample_measurement": {"label": "Hasil Ukur Sample", "ps": "PS-12", "decision": "S-01",
                           "note": "FASE S — kamus bentuk hasil ukur (label · satuan · "
                                   "batas wajar). `sample_types.measurement_fields` hanya "
                                   "menyebut namanya; bentuknya SATU sumber di sini supaya "
                                   "validator server, form setor hasil, dan ringkasan "
                                   "panel tidak punya tiga salinan.",
                           "values": SAMPLE_MEASUREMENTS, "in_use": True},
    "sample_deliver_target": {"label": "Tujuan Kirim Sample", "ps": "PS-12",
                              "decision": "S-02",
                              "note": "FASE S — `md_samples.delivered_to` WAJIB salah satu: "
                                      "pelanggan · sales · supplier (kirim balik) · arsip "
                                      "internal/QC.",
                              "values": SAMPLE_DELIVER_TARGETS, "in_use": True},
