...
SCOPE_FIELD: Dict[str, Optional[str]] = {
    ...
    # FASE S — master JENIS SAMPLING: berlapis global→badan usaha, sama seperti lini
    # & tahapan. Baris `entity_id="all"` = jenis yang berlaku untuk seluruh grup.
    "sample_types": "entity_id",
    # FASE I — master ALASAN KELUHAN RETUR: berlapis global→badan usaha, sama seperti
    # lini · tahapan · jenis sampling. Baris `entity_id="all"` = kode keluhan baku grup.
    "complaint_reasons": "entity_id",
...
SCOPED_COLLECTIONS = {
    ...
    "wms_tasks", "shipments",
    # FASE I — dokumen inspeksi (SPK QC). `qc_inspections` DICABUT pada commit yang
    # sama: koleksi itu terdaftar di sini sejak lama dengan **0 dokumen dan tanpa satu
    # pun penulis** (drift D2). Membiarkan nama mati di registry membuat gate
    # "menjaga" sesuatu yang tidak ada — dan agen berikutnya percaya koleksinya ada,
    # persis yang terjadi pada v1. Namanya juga bukan `qc_*` lagi karena cakupannya
    # lebih luas dari QC: penerimaan PO, hasil makloon, retur, dan barang pengganti.
    "inspections",
    # 2026-06 — JEJAK NILAI (HPP) roll. Ber-`entity_id` karena rollnya milik satu badan
    # usaha; riwayat nilai persediaan PT lain tidak boleh terbaca (endpoint
    # `/api/inventory/rolls/{id}/cost-history` menolak 403 di luar penugasan).
    "roll_cost_history",
    "entity_prices",
    ...
    "process_stages",
    # FASE S — master jenis sampling (berlapis global→badan usaha). Alasannya sama
    # dengan lini & tahapan: jenis khusus satu badan usaha tidak boleh terbaca badan
    # usaha lain, sedangkan baris global tetap terlihat semua lewat
    # `resolve_list_scope_inherit`/`entity_master_service`.
    "sample_types",
    # FASE I — master alasan keluhan retur (berlapis global→badan usaha). Alasannya
    # sama dengan lini · tahapan · jenis sampling: kode keluhan khusus satu badan
    # usaha tidak boleh terbaca badan usaha lain, sedangkan baris global tetap
    # terlihat semua lewat `resolve_list_scope_inherit`/`entity_master_service`.
    "complaint_reasons",
...
INHERITED_GLOBAL_VALUES: Dict[str, List[Any]] = {
    ...
    # FASE S — jenis sampling: baris global "all" WAJIB tetap terlihat di semua badan
    # usaha. Tanpa baris ini, form Permintaan Sample tidak menawarkan satu jenis pun
    # dan pekerjaan sampling berhenti sama sekali.
    "sample_types": ["all", "", None],
    # FASE I — alasan keluhan retur: baris global "all" WAJIB tetap terlihat di semua
    # badan usaha. Tanpa baris ini, form retur tidak menawarkan satu sebab pun dan
    # lembar retur kembali menjadi "barang kembali" tanpa alasan yang bisa dihitung.
    "complaint_reasons": ["all", "", None],
}
...
def assert_active_entity_access(doc: Dict[str, Any], collection: str, ctx: EntityContext) -> None:
    """Isolasi detail SEKETAT daftarnya: dokumen harus milik badan usaha AKTIF.

    `assert_entity_access` hanya memeriksa PENUGASAN akun (`allowed_entity_ids`), jadi
    akun multi-entitas yang sedang berdiri di KSC tetap bisa membuka dokumen KANDA
    lewat id — padahal daftarnya sudah menyembunyikannya (T3 audit training 2026-06).
    Mode gabungan (`X-Entity-Id: all`) kembali ke pagar penugasan.
    """
    assert_entity_access(doc, collection, ctx)
    fld = field_for(collection)
    if fld is None or not doc or ctx.view_all:
        return
    ent = doc.get(fld)
    if ent and ctx.active_entity_id and ent != ctx.active_entity_id:
        raise HTTPException(status_code=404, detail="Data tidak ditemukan untuk entitas ini")
