...
from services import roll_cost_history as _roll_cost_history
...
async def roll_cost_history_endpoint(roll_id: str, request: Request) -> Dict[str, Any]:
    ...
    KENAPA ADA (2026-06): nilai sebuah roll bisa berubah dari beberapa arah (migrasi
    startup, revaluasi retur/pembelian antar-PT, voucher biaya masuk). Selama
    perubahan itu tak berjejak, satu-satunya cara mengetahui "kenapa roll ini bernilai
    ...
    ctx = await entity_ctx(request)
    owner = roll.get("owner_entity_id") or roll.get("entity_id") or ""
    if owner and not ctx.can_access(owner):
        raise HTTPException(status_code=403, detail="Roll ini milik badan usaha lain.")
    rows = await _roll_cost_history.history(roll_id)
    return {"roll_id": roll_id, "roll_no": roll.get("roll_no", ""),
            "unit_cost": float(roll.get("unit_cost") or 0),
            "base_unit_cost": float(roll.get("base_unit_cost") or 0),
            "length_remaining": float(roll.get("length_remaining") or 0),
            "cost_basis": roll.get("cost_basis") or {},
