...
class POStagePatch(BaseModel):
    """FASE P — tandai satu tahap PO di papan per lini.

    `stage_code` WAJIB salah satu tahap pada urutan lini dokumen itu (dibaca dari
    master `product_lines.stage_sequence`, BUKAN dari daftar di kode ini — kalau
    ditulis di sini, lini baru yang ditambah pemilik akan ditolak validasi tanpa
    sebab yang bisa dijelaskan). `note` opsional: alasan/keterangan yang tampil di
    riwayat PO."""
    stage_code: str
    status: str = "done"              # pending | in_progress | done
    note: str = ""
