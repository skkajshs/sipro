...
from routers import (
    ...
    po_board,        # FASE P — papan PO per lini (urutan pendaftaran penting)
    ...
    inspections,     # FASE I — Inspeksi & QC sebagai dokumen (`<ENT>/INS-#####`)
...
for module in [
    ...
    inventory, sales_orders_extra, sales_orders, invoices, wms, documents, admin,
    reporting, audit, cycle_count, onboarding, label_printer, transfers,
    # FASE P — papan PO per lini. DIDAFTARKAN SEBELUM `purchase_orders` supaya
    # `GET /purchase-orders/board` tidak ditelan `GET /purchase-orders/{po_id}`
    # (pola yang sama dengan `purchase_orders_extra` untuk `/blanket`).
    po_board,
