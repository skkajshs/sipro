...
from services import po_board_service as _po_board       # FASE P — papan PO per lini
...
async def create_blanket(payload, actor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Satuan kontrak WAJIB dari master (sama seperti PO standar): tanpa ini drawdown
    # called/remaining membandingkan qty beda satuan tanpa konversi.
    ...
    for it in payload.items:
        ...
        unit = (it.unit or base_unit).strip()
        if unit.lower() != (base_unit or "meter").strip().lower():
            try:
                await uomr.convert_with_trail(p, float(it.contract_qty), unit, base_unit,
                                              engine=_uom_engine, context="blanket_po", line=it)
            except uomr.UomRuleError as exc:
                raise HTTPException(
                    status_code=400,
                    detail=f"Item {p.get('sku')}: satuan '{unit}' tidak sah — {exc}") from exc
        items.append({
    ...
    doc = {
        ...
        **_pr_sourcing.PO_ORIGIN_EMPTY,
        # FASE P — kontrak payung TIDAK muncul di papan per lini (ia perjanjian harga,
        # bukan pesanan yang menempuh tahap), tetapi field-nya tetap ADA & kosong
        # supaya tidak ada dokumen `purchase_orders` yang bentuknya berbeda.
        **_po_board.PO_BOARD_EMPTY,
        "source": "blanket",
...
async def prepare_call_off(blanket_id: str, payload, actor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for it in payload.items:
        ...
        # Satuan call-off HARUS sama dengan satuan kontrak: drawdown (sisa qty) dan
        # harga sepakat dihitung per satuan kontrak — beda satuan = salah hitung diam-diam.
        co_unit = (it.unit or "").strip()
        if co_unit and co_unit.lower() != str(ci["unit"]).strip().lower():
            raise HTTPException(
                status_code=400,
                detail=f"Item {ci['sku']}: satuan call-off '{co_unit}' harus sama dengan "
                       f"satuan kontrak '{ci['unit']}'.")
