...
async def amend_po(po_id: str, payload, actor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # INV-REF-04 — task inbound adalah dokumen bernomor (`grn`) yang SUDAH bertaut
    # dua arah ke PO-nya di titik lahirnya. Menghapusnya tanpa menyapu tautan balik
    # meninggalkan PO hidup yang memegang ref ke penerimaan yang tak ada lagi, dan
    # layar "Relasi Dokumen" menawarkan tautan menuju 404. Sapu dulu, hapus kemudian.
    _dead_tasks = [t["id"] async for t in db.wms_tasks.find(
        {"po_id": po_id, "flow_type": "inbound", "received_qty": {"$lte": 0},
         "status": {"$nin": ["completed", "cancelled"]}}, {"_id": 0, "id": 1})]
    for _tid in _dead_tasks:
        await _refs.safe_unlink_all("grn", _tid)
