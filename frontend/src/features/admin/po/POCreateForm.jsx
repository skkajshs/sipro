...
export default function POCreateForm({
  ...
  function handleItemProductSelect(v) {
    ...
    // Default grade dari master produk (bisa diganti) — dulu kosong tanpa label,
    // pengguna baru tahu wajib saat "Tambah Item" ditolak.
    const defGrade = newItem.expected_grade || prod?.default_grade || prod?.grade || "";
    setNewItem({ ...newItem, product_id: v, unit: baseUnit, expected_grade: defGrade });
    ...
    <div data-testid="create-po-form" className={isModal ? "" : "section-card mb-3"}>
      ...
      <div className={isModal ? "space-y-3" : "section-body space-y-3"}>
        ...
        <div className="bg-[#FAFBFC] rounded-md border border-[#EFF0F2] p-2.5">
          ...
          {/* Label kolom mini — dulu hanya placeholder, "Grade *" sering terlewat. */}
          <div className="grid grid-cols-[1fr_68px_130px_56px_88px_52px_130px_auto] gap-2 mb-0.5 px-0.5 text-[9px] font-bold uppercase text-[#9A9BA3]">
            <span>Produk</span><span>Qty</span><span>Satuan</span><span>Roll</span><span>Harga</span><span>Disc%</span><span>Grade <span className="text-red-500">*</span></span><span></span>
          </div>
          <div className="grid grid-cols-[1fr_68px_130px_56px_88px_52px_130px_auto] gap-2">
