...
export function useAppActions(state) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setNotifications(Array.isArray(notifResp.data?.items) ? notifResp.data.items : []);
      setNotifMeta?.({ page: notifResp.data?.page || 1,
                       total: notifResp.data?.total || 0,
                       has_more: Boolean(notifResp.data?.has_more) });
  ...
  // BERHALAMAN (2026-06): daftar dulu dipotong keras di 100 baris tanpa halaman
  // berikutnya, sementara lencana menghitung SEMUA yang belum dibaca — begitu kotak
  // melewati 100 pesan, angka di lencana tak lagi bisa dibuktikan dari layar.
  const NOTIF_PAGE_SIZE = 20;
  const loadNotifications = useCallback(async () => {
    const params = { page: 1, page_size: NOTIF_PAGE_SIZE };
  ...
  /** Halaman berikutnya lonceng — ditambahkan ke daftar (tidak menggantinya). */
  const loadMoreNotifications = useCallback(async (nextPage) => {
    const params = { page: nextPage, page_size: NOTIF_PAGE_SIZE };
    if (selectedEntity && selectedEntity !== "all") params.entity_id = selectedEntity;
    try {
      ...
      setNotifMeta?.({ page: res.data?.page || nextPage,
